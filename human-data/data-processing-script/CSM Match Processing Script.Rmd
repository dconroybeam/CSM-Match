...
data <- read.csv('human-data/raw-data/CSM Match Post-Date Raw 012826.csv')
...
#average amount of days between dates

avgDayDiff <- data %>%
  group_by(PIN) %>%
  filter(sum(!is.na(StartDate)) == 2) %>% #keep IDs w/ two dates only
  arrange(PIN, StartDate) %>%
  summarise(dayDifference = as.numeric(max(StartDate) - min(StartDate)), .groups = "drop")
MeanAvgDayDiff <- mean(avgDayDiff$dayDifference, na.rm = TRUE)
