...
#Load the data
data<-read.csv("human-data/processed-data/CSM-Match-matching-data-S24.csv")
...
mvCalc <- function(ideal,
  ...
  mv <- (-1 * mv + sqrt(10 ^ 2 * 20)) / sqrt(10 ^ 2 * 20)
...
mateChoice <- function(data) {
  ...
  #Randomly sample from the more numerous sex to avoid sex ratio bias issues
  if(nFemales>nMales){
    females<-females[sample(1:nrow(females),nMales,replace=F),]
  } else {
    males<-males[sample(1:nrow(males),nFemales,replace=F),]
  }
