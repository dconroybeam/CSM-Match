...
library(simr) #for power simulations (powerSim function)
...
##Exploratory analysis 8 (prefMatch w/ time 1 preferences and date's reports of their own traits)
...
#Exploratory analysis 8: is physical attractiveness driving all effects
...
#exploratory analysis 9: model with perceived and actual match together 
attractionModelPerceivedAndActual <- lmer(scale(ltAttraction) ~ scale(prefMatch) + scale(prefMatchActual) + (1|PIN) + (1|mPIN), data = dataComplete) #standardized outcome
...
##Exploratory prediction 3: Romantic attraction & second date interest as a function of prefmatch and perceived date attraction
...
dataMelt$cardUseBinned <- ntile(dataMelt$cardUse, 2)

dataMelt <- dataMelt %>%
  mutate(cardUseBinned = ntile(cardUse, 2),
         cardUseBinned = factor(cardUseBinned,
                                levels = c(1, 2),
                                labels = c("Below Median", "Above Median"))) %>%
  filter(!is.na(cardUseBinned))
...
#not relevant for current study, just for potential follow up
