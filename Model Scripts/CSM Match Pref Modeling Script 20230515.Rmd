...
#Human Data#
#Load in data
data<-read.csv(file="Human Data/Processed Data/CAREER2P PROCESSED Data 08262020 114326.csv")
...
prefNames<-tools::toTitleCase(gsub("ideal_","",colnames(data[,11:26])))
...
mrPath<-paste0("Model Outputs/CSM Match Pref Modeling Outputs Pref",numPrefs," N",n," .csv")

if(file.exists(mrPath)==F){
  
  c("loop","numPrefs",paste0("pref", 1:numPrefs), "acc")
    output<-data.frame(matrix(NA,1,3+numPrefs))
    colnames(output)<-c("loop","numPrefs",paste0("pref", 1:numPrefs), "acc")
...
mvCalc<-function(ideal,traits){
  ...
  mv<-apply(traits,1,function(x) 10*(-1*(dist(rbind(ideal,x)))+sqrt(10^2*length(ideal)))/sqrt(10^2*length(ideal)))
...
mateChoice<-function(females,males,prefs){
  ...
  maleAttMatrix<-t(apply(males,1,function(q) mvCalc(q[(prefs+10)],females[,(prefs+26)])))
  ...
  femaleAttMatrix<-t(apply(females,1,function(q) mvCalc(q[(prefs+10)],males[,c(prefs+26)])))
  ...
  acc<-mean(pairs[match(trueRels[,1],pairs[,2]),44]==trueRels[,2],na.rm=T)
...
#Select a subsample of participants
...
#Determine which preferences to start
...
#Create a vector to store the number of times a preference appeared in a winning set
prefRecord <- rep(0, 16)

#Create a dataframe to store accuracy as a function of preference set
...
while (sum(rowSums(is.na(accRecord))<13) < loops) {
  ...
  #Randomly change one preference from the starting set to make the testing set
  testPrefs <- startPrefs
  testPrefs[sample(1:length(testPrefs), 1)] <- sample((1:16)[-testPrefs], 1)
  ...
  #Otherwise, switch with a 5% chance to avoid local optima
  if (accRatio > 1 | rbinom(1, 1, .05)) {
  ...
  #Record which preferences won this competition
  prefRecord[startPrefs] <- prefRecord[startPrefs] + 1
  
  #Store the current winning preferences and accuracy
  accRecord <- rbind(accRecord,
    c(l, numPrefs,prefNames[sort(startPrefs)], round(startAcc * 100, 2)))
    
  #Save winning preferences and accuracy
  write.table(accRecord[nrow(accRecord),],file=mrPath,append=T,sep=",",row.names=F,col.names=F)
...
accPlot<-ggplot(data=accRecord,aes(x=loop,y=acc,group=1))+
  labs(x="Loop",y="Simulation Accuracy")+
  geom_line(color="dodgerblue3",lwd=1.5)+
